<template>
  <div class="main-content">
    <breadcumb :page="$t('Warehouse_AddProduct')" :folder="$t('Products')" />
          ...
          <b-col md="8" class="mb-2">
                ...
                <b-col md="6" class="mb-2">
                  <validation-provider name="Image" ref="Image" rules="mimes:image/*">
                    <b-form-group slot-scope="{validate, valid, errors }" label="Product Image">
                      <input :state="errors[0] ? false : (valid ? true : null)" :class="{ 'is-invalid': !!errors.length }"
                        @change="onFileSelected" label="Choose Image" type="file">
                  ...
                  <table class="table table-hover table-sm">
                    ...
                    <tbody>
                      ...
                      <tr v-for="materiel in materiels">
                        ...
                        <td class="text-right">{{ currentUser.currency }} {{ formatNumber(materiel.cost *
                          materiel.quantity, 2)}}</td>

                        <td class="text-right">

                          <a style="color: #ffff;" @click="delete_materiel(materiel.product_id)"
                  ...
                  <validation-provider name="Type" :rules="{ required: true }">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('type') + ' ' + '*'">
                      <v-select :class="{ 'is-invalid': !!errors.length }"
                        ...
                        @input="Selected_Type_Product" :reduce="label => label.value" :placeholder="$t('type')"
                        :options="[
                ...
                <b-col md="6" class="mb-2"
                  ...
                  <validation-provider name="Product Price" :rules="{ required: true, regex: /^\d*\.?\d*$/ }"
                    ...
                    <b-form-group :label="$t('ProductPrice') + ' ' + '*'">
                      ...
                      <b-form-invalid-feedback id="ProductPrice-feedback">{{ validationContext.errors[0]
                        }}</b-form-invalid-feedback>
                    </b-form-group>
                  </validation-provider>
                </b-col>
                ...
                <b-col md="6" class="mb-2" v-if="product.type != 'is_service'">
                  <validation-provider name="Unit Product" :rules="{ required: true }">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('UnitProduct') + ' ' + '*'">
                      <v-select :class="{ 'is-invalid': !!errors.length }"
                        :state="errors[0] ? false : (valid ? true : null)" v-model="product.unit_id" class="required"
                        required @input="Selected_Unit" :placeholder="$t('Choose_Unit_Product')"
                  ...
                  <validation-provider name="Stock Alert" :rules="{ regex: /^\d*\.?\d*$/ }" v-slot="validationContext">
                    <b-form-group :label="$t('StockAlert')">
                      <b-form-input :state="getValidationState(validationContext)"
                        ...
                        :placeholder="$t('Enter_Stock_alert')" v-model="product.stock_alert"></b-form-input>
            ...
            <b-card class="mt-3">
              <b-row>
                <!-- Bulk Unit  -->
                <b-col md="6" class="mb-2">
                  <validation-provider name="Bulk Unit" :rules="{ required: true }">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Bulk_Unit') + ' ' + '*'">
                      <v-select :class="{ 'is-invalid': !!errors.length }"
                        :state="errors[0] ? false : (valid ? true : null)" v-model="product.type"
                        @input="Selected_Type_Product" :reduce="label => label.value" :placeholder="$t('Bulk_Unit')"
                        :options="[
                            { label: 'Bulk Unit', value: 'is_single' },
                            { label: 'Carton', value: 'is_variant' },
                            { label: 'Bag', value: 'is_service' },
                            { label: 'Case', value: 'is_combo' }
                          ]"></v-select>
                      <b-form-invalid-feedback>{{ errors[0] }}</b-form-invalid-feedback>
                    </b-form-group>
                  </validation-provider>
                ...
                <b-col md="6" class="mb-2">
                  <validation-provider name="Individual Unit Type" :rules="{ required: true }">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Individual_Unit') + ' ' + '*'">
                      <v-select :class="{ 'is-invalid': !!errors.length }"
                        :state="errors[0] ? false : (valid ? true : null)" v-model="product.type"
                        @input="Selected_Type_Product" :reduce="label => label.value" :placeholder="$t('Individual_Unit')"
                        :options="[
                            { label: 'Select Unit Type', value: 'is_single' },
                            { label: 'Kg', value: 'is_variant' },
                            { label: 'Pc', value: 'is_service' },
                ...
                <b-col md="6" class="mb-2"
                  ...
                  <validation-provider name="Product Cost" :rules="{ required: true, regex: /^\d*\.?\d*$/ }"
                    v-slot="validationContext">
                    <b-form-group :label="$t('Product_Cost') + ' ' + '*'">
                      <b-form-input :state="getValidationState(validationContext)"
                        aria-describedby="ProductPrice-feedback" label="Price" :placeholder="$t('Enter_Product_Cost')"
                        v-model="product.price"></b-form-input>
                ...
                <b-col md="6" class="mb-2"
                  v-if="product.type == 'is_single' || product.type == 'is_service' || product.type == 'is_combo'">
                  <validation-provider name="Individual Weight" :rules="{ required: true, regex: /^\d*\.?\d*$/ }"
                    v-slot="validationContext">
                    <b-form-group :label="$t('Individual_Weight') + ' ' + '*'">
                      <b-form-input :state="getValidationState(validationContext)"
                        aria-describedby="ProductPrice-feedback" label="Price" :placeholder="$t('Enter_Product_Cost')"
                        v-model="product.price"></b-form-input>
  ...
  >
    ...
    >
      {{ month }}
              ...
              <b-col md="6" class="mb-2" v-if="product.type != 'is_service'">
                  <validation-provider name="Shelf" :rules="{ required: true }">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Shelf') + ' ' + '*'">
                      <v-select :class="{ 'is-invalid': !!errors.length }"
                        :state="errors[0] ? false : (valid ? true : null)" v-model="product.unit_sale_id"
                        :placeholder="$t('Shelf_Choose')" :reduce="label => label.value"
                        :options="units_sub.map(units_sub => ({ label: units_sub.name, value: units_sub.id }))" />
                      <b-form-invalid-feedback>{{ errors[0] }}</b-form-invalid-feedback>
                    </b-form-group>
                  </validation-provider>
